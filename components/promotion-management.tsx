...
export function PromotionManagement() {
  ...
  const [templateList, setTemplateList] = useState<string | null>(null);
    ...
    axios.get(`${process.env.NEXT_PUBLIC_API_BACKEND}/promotion/data`).then(
      res => {
        ...
        if (res.data != null) {
          ...
          console.log('Success' + JSON.stringify(res.data.promotions))
        } else {
  ...
  const handlePausePromotion = async (id: number) => {
    try {
      const res = await axios.post(`${process.env.NEXT_PUBLIC_API_BACKEND}/promotion/setStatus`, {
        id,
        status: "PAUSED",
      });
                          ...
                          <TableCell colSpan={8} className="bg-gray-50">
                            {actionType === "view" ? (
                              ...
                              <form onSubmit={(e) => handleSubmit(e, "update")} className="grid grid-cols-3 gap-6 text-sm bg-white p-6 rounded-lg border shadow-sm">
                                ...
                                <div>
                                  <label className="block text-gray-600 mb-1 font-medium">Giá trị percent or fixed</label>
                                  <input
                                    ref={valueRef}
                                    type="number"
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 bg-gradient-to-r from-[#001F54]/10 to-[#4DD0E1]/10 rounded-lg">
                    <div>
                      ...
                      <div className="text-2xl font-bold text-[#001F54]">211%</div>
                    </div>
